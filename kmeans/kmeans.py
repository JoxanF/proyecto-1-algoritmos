...
def kmeans(x,k, no_of_iterations, measurementData):
    idx = np.random.choice(len(x), k, replace=False) # +1
    #Randomly choosing Centroids 
    centroids = x[idx, :] #Step 1 +1
     
    #finding the distance between centroids and all the data points
    distances = cdist(x, centroids ,'euclidean') #Step 2  +1
     
    #Centroid with the minimum Distance
    points = np.array([np.argmin(i) for i in distances]) #Step 3 +1
    ...
    #Step 4
    for _ in range(no_of_iterations): # +n
        centroids = [] #+n
        for idx in range(k): #+n^2
            #Updating Centroids by taking mean of Cluster it belongs to
            temp_cent = x[points==idx].mean(axis=0) #+n^2
            centroids.append(temp_cent) #+n^2
            # measurement
            measurementData[0] += 3 
            measurementData[2] += 3 
            # measurement
 
        centroids = np.vstack(centroids) #Updated Centroids #+n
         
        distances = cdist(x, centroids ,'euclidean') #+n
        points = np.array([np.argmin(i) for i in distances]) #+n
    ...
    # +1+1+1+1+n+n+n^2+n^2+n^2+n+n+n+1 = 5+5n+3n^2
